  ...
  if (!food) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 px-6">
        ...
        <Link to="/home" className="text-sm font-semibold text-primary">Go to menu</Link>
      </div>
    );
  }

  const total = food.price * qty;
  const protein = Math.round(food.calories * 0.06);
  const carbs = Math.round(food.calories * 0.08);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background pb-28">
      ...
      <div className="px-5 pt-5">
        <h1 className="text-[22px] font-extrabold leading-tight text-foreground">{food.name}</h1>
        ...
        <h3 className="mt-5 text-[15px] font-bold text-foreground">Nutrition</h3>
        <div className="mt-2 flex gap-2">
          {[
            { e: "🔥", v: `${food.calories} cal` },
            { e: "🥩", v: `${protein}g protein` },
            { e: "🍞", v: `${carbs}g carbs` },
          ].map((n) => (
            <div
              key={n.v}
              className="flex-1 rounded-xl bg-card px-2 py-2.5 text-center text-[11px] font-medium text-foreground"
        ...
        <div className="mt-2 inline-flex items-center gap-3 rounded-full bg-card p-1.5" style={{ boxShadow: "var(--shadow-card)" }}>
          <button
